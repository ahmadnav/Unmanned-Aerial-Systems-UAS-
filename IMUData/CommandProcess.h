...
//const int maxbyteIMU = 4;//IMU payload length for each of IMU vectors, x ,y, and z orientations
//#pragma endregion
//
...
//const int maxbyteGPS = 4;//GPS payload length for lat, and long
//#pragma endregion
...
public:
	static const uint8_t MAX_SIZE = 0xFF;//255 

	uint8_t outgoingmessage[MAX_SIZE];

	static const int maxbyteGPS = 4;//GPS payload length for lat, and long
	static const int maxbyteIMU = 4;//IMU payload length for each of IMU vectors, x ,y, and z orientations
	...
	void insertbytearray(byte* arr, byte* ins_arr,int ins_loc, int size);
};
